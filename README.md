...
Customer POC lab materials — demonstrating that Sangfor NGFW (built-in WAF module + IPS + App Control) detects and blocks OWASP web attacks and network-level attacks launched from Kali against DVWA.
...
| VULNSERVER | 192.168.20.0/24 | DVWA server 192.168.20.10 |
| LAN | 192.168.10.0/24 | Client VM (sanity check) 192.168.10.10 |
| MGMT | 172.16.1.0/24 | NGFW management 172.16.1.1 |

Traffic: Kali (WAN) → NGFW public IP 10.0.0.1:80 → DNAT → DVWA (192.168.20.10:80), all inspected by WAF + IPS.

## Files

| File | Purpose |
|------|---------|
| `Sangfor-NGFW-WAF-POC-LAB-Design-2026-09-16.md` | Full lab design: topology, IP scheme, NGFW config steps, test matrix (5 phases), pass criteria, evidence checklist |
| `deploy-vulnserver.sh` | Ubuntu 24.04: install Docker, run DVWA, auto-init database, set security level to low |
...
| `auto-attack.sh` | One-command automated attack run — takes only the target public IP as input |
| `README-deploy.md` | Step-by-step deployment on Sangfor HCI (VMs, zones, NGFW config, GitHub-pull method) |

## Quick Start

### 1. Deploy the vuln server (Ubuntu 24.04 VM)
...
What it does (per phase): login DVWA → nmap recon → sqlmap SQLi → XSS / LFI / command injection → webshell upload → hydra brute force (top 200 passwords) → reverse shell (auto listener + trigger + connection check).
...
| Phase | Attack | Tool | Expected NGFW Action |
|-------|--------|------|----------------------|
| 1 | Port scan / recon | nmap | IPS scan detection |
| 2 | SQLi, XSS, LFI, CMDi, webshell upload | sqlmap, curl, Burp | WAF rule hit — blocked |
| 3 | Login brute force | hydra | WAF login protection / rate limiting |
| 4 | Reverse shell (C2 egress) | netcat, php | IPS / App Control flag |
| 5 | Legit browsing (LAN client) | browser | No blocking (sanity check) |
